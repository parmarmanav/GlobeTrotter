...
export function StopsList({
  ...
  return (
    <div className="space-y-4">
      ...
      {stops.length === 0 ? (
        <div className="p-6 text-center bg-slate-800/50/70 rounded-2xl border border-dashed border-[var(--color-border-subtle)]">
          <MapPin className="w-8 h-8 text-indigo-400/60 mx-auto mb-2" />
          <p className="text-xs font-bold text-slate-300">No destination stops added</p>
        ...
        <div className="space-y-2.5">
          {stops.map((stop, idx) => {
            ...
            return (
              <div
                key={stopId}
                className="group flex items-center justify-between p-3 rounded-xl bg-[var(--color-card)] border border-[var(--color-border-subtle)] hover:border-teal-300 shadow-2xs transition-all"
              >
                <div className="flex items-center gap-3 min-w-0">
                  {/* Sequence number badge */}
                  <div className="w-6 h-6 rounded-lg bg-indigo-500/10 text-indigo-400 font-bold text-xs flex items-center justify-center shrink-0 border border-teal-200/60">
                    {idx + 1}
                  </div>
                  ...
                  <div className="min-w-0">
                    ...
                    {dateRange && (
                      <p className="text-[10px] text-slate-400 flex items-center gap-1 mt-0.5">
                        <Calendar className="w-2.5 h-2.5 text-indigo-400" /> {dateRange}
                      </p>
                    )}
                    {stop.notes && (
